...
def train(cfg, train_loader, model, criterion, optimizer, scaler, epoch, num_batch, num_warmup,
    ...
    for i, (input, target, paths, shapes) in enumerate(train_loader):
        ...
        lo = head_losses[0] + head_losses[1] + head_losses[2]
        lossesss.append(lo)
        ld = head_losses[3]
        lossesss.append(ld)
        ll = head_losses[4] + head_losses[5]
        lossesss.append(ll)

        for l in range(num_tasks):
            scaler.scale(lossesss[l]).backward(retain_graph=True)
        ...
        grads_task = torch.stack(grads_task, dim=0)
            ...
            if config.TEST.PLOTS:
                if batch_i <=500:
                    for i in range(test_batch_size):
                        ...
                        _ = show_seg_result(img_test, da_seg_mask, i, batch_i, save_dir)
                        _ = show_seg_result(img_test1, da_gt_mask, i, batch_i, save_dir, is_gt=True)
                        ...
                        # plot_img_and_mask(img_test, seg_mask, i,epoch,save_dir)
                        img_ll1 = img_ll.copy()
                        _ = show_seg_result(img_ll, ll_seg_mask, i, batch_i, save_dir, is_ll=True)
                        _ = show_seg_result(img_ll1, ll_gt_mask, i, batch_i, save_dir, is_ll=True, is_gt=True)
                        ...
                        cv2.imwrite(save_dir + "/batch_{}_{}_det_pred.png".format(batch_i, i), img_det)
                        img_r = cv2.imread(paths[i])
                        img_det1 = show_seg_result(img_r, (da_seg_mask, ll_seg_mask), _, _, is_demo=True)
                        if len(det):
                            det[:, :4] = scale_coords([720, 1280], det[:, :4], img_det1.shape).round()
                        for *xyxy, conf, cls in reversed(det):
                            # print(cls)
                            label_det_pred = f'{names[int(cls)]} {conf:.2f}'
                            plot_one_box(xyxy, img_det1, label=label_det_pred, color=colors[int(cls)], line_thickness=3)
                        cv2.imwrite(save_dir + "/batch_{}_{}_result.png".format(batch_i, i), img_det1)
                        ...
                        cv2.imwrite(save_dir + "/batch_{}_{}_det_gt.png".format(batch_i, i), img_gt)
